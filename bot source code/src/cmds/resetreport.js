...
module.exports = {
	name:"resetreport",
	aliases: ["reset"], 
	description:"Reset the call admin report.",
    access: "moderator",
            ...
            message.channel.messages.fetch(msgid)
  				.then (fetchedMsg => {
                    ...
                    if(currentEmbed.fields.length >= 15)  {
                        ...
                        .then((status) => {
                            if(!status) return message.reply('The report isnt closed or already been reset.');
                            ...
                            .then((status1) => {
                                if(!status1) {
                                    ...
                                    .then((status2) => {
                                        if(status2) {
                                            guildData.set_messagestatus(fetchedMsg.id, false)
                                            .then(() => {
                                                fetchedMsg.edit({content: `A new report has been made.`, embeds: [embednamee]});
                                            	fetchedMsg.reply({  content: "Done!" }); 
                                            	return;
                                            })
